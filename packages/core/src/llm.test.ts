...
describe("OpenAICompatibleChatProvider", () => {
  test("uses DeepSeek defaults and sends OpenAI-compatible chat requests", async () => {
    ...
    const result = await provider.chat({ messages: [{ role: "user", content: "hello" }], maxTokens: 16 });
