...
export class SunPilotRuntime {
  ...
  async createRun(input: unknown, workflowId = "fixture.echo", mode: RunMode = "approval_required"): Promise<RunRecord> {
    ...
    await this.db.insertJob({ id: `job_${crypto.randomUUID()}`, runId: run.id, status: "pending", payload: { workflowId, mode } });
    await this.emit({ runId: run.id, type: "run.created", payload: run });
    await this.emit({ runId: run.id, type: "workflow.selected", payload: { workflowId } });
    ...
    for (const planned of plan.steps) {
      ...
      await this.emit({ runId: run.id, stepId: step.id, type: "step.created", payload: step });
    }
    ...
    if (mode === "dry_run") {
      ...
      await this.db.updateRunStatus(run.id, "completed", new Date().toISOString());
      await this.db.updateJobStatus(run.id, "completed");
      await this.emit({ runId: run.id, type: "run.completed", payload: { dryRun: true } });
  ...
  async approve(approvalId: string, decision: unknown): Promise<ApprovalRecord> {
    ...
    const approval = await this.db.decideApproval(approvalId, "approved", decision);
    if (!approval) throw await this.approvalDecisionError(approvalId);
    await this.db.audit({ runId: approval.runId, stepId: approval.stepId, actor: "local-user", action: "approval.approve", target: approval.id, risk: approval.risk, payload: decision });
    await this.emit({ runId: approval.runId, stepId: approval.stepId, type: "approval.approved", payload: approval });
    if (approval.stepId) {
      const step = (await this.db.listSteps(approval.runId)).find((item) => item.id === approval.stepId);
      if (step?.type === "approval") {
        await this.db.updateStep(approval.stepId, "completed", { approved: true });
        await this.emit({ runId: approval.runId, stepId: approval.stepId, type: "step.completed", payload: { approvalId } });
  ...
  async reject(approvalId: string, decision: unknown): Promise<ApprovalRecord> {
    await this.ensureApprovalCanBeDecided(approvalId);
    const approval = await this.db.decideApproval(approvalId, "rejected", decision);
    if (!approval) throw await this.approvalDecisionError(approvalId);
    await this.db.audit({ runId: approval.runId, stepId: approval.stepId, actor: "local-user", action: "approval.reject", target: approval.id, risk: approval.risk, payload: decision });
    await this.emit({ runId: approval.runId, stepId: approval.stepId, type: "approval.rejected", payload: approval });
    if (approval.stepId) await this.db.updateStep(approval.stepId, "canceled", undefined, { rejected: true });
    await this.db.updateRunStatus(approval.runId, "canceled", new Date().toISOString());
    await this.db.updateJobStatus(approval.runId, "canceled");
    await this.emit({ runId: approval.runId, type: "run.canceled", payload: { approvalId } });
    return approval;
  }
  ...
  async interrupt(runId: string): Promise<RunRecord> {
    ...
    await this.db.updateRunStatus(runId, "interrupted", new Date().toISOString());
    ...
    for (const step of await this.db.listSteps(runId)) {
      if (!["completed", "failed", "skipped", "canceled", "interrupted"].includes(step.status)) {
        await this.db.updateStep(step.id, "interrupted", undefined, { reason: "run interrupted" });
        await this.emit({ runId, stepId: step.id, type: "step.interrupted", payload: { actor: "local-user" } });
      }
    }
    await this.emit({ runId, type: "run.interrupted", payload: { actor: "local-user" } });
  ...
  async cancel(runId: string): Promise<RunRecord> {
    ...
    for (const step of await this.db.listSteps(runId)) {
      if (!["completed", "failed", "skipped", "canceled", "interrupted"].includes(step.status)) {
        await this.db.updateStep(step.id, "canceled", undefined, { reason: "run canceled" });
      }
    }
    await this.db.audit({ runId, actor: "local-user", action: "run.cancel", target: runId, payload: { actor: "local-user" } });
    await this.emit({ runId, type: "run.canceled", payload: { actor: "local-user" } });
    return (await this.db.getRun(runId))!;
  }
  ...
  async retry(runId: string): Promise<RunRecord> {
    ...
    await this.db.audit({ runId, actor: "daemon", action: "run.retry", target: retry.id, payload: { sourceRunId: runId } });
    await this.emit({ runId, type: "run.interrupted", payload: { retryRunId: retry.id, reason: "retry requested" } });
    return retry;
  }
  ...
  private async continueRun(runId: string): Promise<void> {
    ...
    for (const step of steps) {
      ...
      if (step.type === "skill") {
        ...
        if (this.requiresApproval(capability) && !(await this.hasApprovedCapability(step))) {
    ...
    await this.emit({ runId, type: "run.completed", payload: { artifacts: await this.db.listArtifacts(runId) } });
  }

  private async requestApproval(step: StepRecord, capability?: ToolCapability): Promise<ApprovalRecord> {
    const existing = (await this.db.listApprovals()).find((approval) => approval.stepId === step.id && approval.status === "pending");
    ...
    await this.db.audit({ runId: step.runId, stepId: step.id, actor: "daemon", action: "approval.request", target: approval.id, risk: approval.risk, payload: approval.requestedAction });
    await this.emit({ runId: step.runId, stepId: step.id, type: "approval.requested", payload: approval });
    return approval;
  }
  ...
  private async ensureApprovalCanBeDecided(approvalId: string): Promise<ApprovalRecord> {
  ...
  private async ensureApprovalRunCanBeDecided(approval: ApprovalRecord): Promise<void> {
  ...
  private async executeSkillStep(step: StepRecord): Promise<void> {
    ...
    await this.emit({ runId: step.runId, stepId: step.id, type: "step.started", payload: step });
    await this.emit({ runId: step.runId, stepId: step.id, type: "skill.execution.started", payload: { skillId: step.skillId, capability: step.capability } });
    try {
      ...
      await this.emit({ runId: step.runId, stepId: step.id, type: "skill.execution.completed", payload: output });
      await this.emit({ runId: step.runId, stepId: step.id, type: "step.completed", payload: output });
    } catch (error) {
      ...
      await this.db.updateRunStatus(step.runId, "failed", new Date().toISOString());
      await this.db.updateJobStatus(step.runId, "failed");
      await this.emit({ runId: step.runId, stepId: step.id, type: "skill.execution.failed", payload });
      await this.emit({ runId: step.runId, stepId: step.id, type: "step.failed", payload });
      await this.emit({ runId: step.runId, type: "run.failed", payload });
      throw error;
    }
  }

  private async emit(input: Omit<SunPilotEvent, "id" | "createdAt">): Promise<void> {
